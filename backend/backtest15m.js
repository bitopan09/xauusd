// V4-Plus 15m Backtest — uses cached 15m data directly
const fs = require('fs');
const path = require('path');

async function runBacktest15m() {
    ...
    if (!fs.existsSync(cacheFile15)) {
        console.error('ERROR: 15m cache file not found at', cacheFile15);
        process.exit(1);
    }

    const cached = JSON.parse(fs.readFileSync(cacheFile15, 'utf8'));
    console.log(`Loaded ${cached.length} cached 15m candles\n`);

    // Convert to Bybit raw format for clientCandles
    const clientCandles = cached.slice().map(k => [
    ...
    const result = await bot.runBacktest(90, 'default', clientCandles, '15', START_EQ);
    const { trades } = result;
    ...
    // Summary line
    const intervalInfo = `15m (${cached.length} candles, ${((cached[0] && new Date(cached[0].timestamp)) || '').toISOString().split('T')[0]} to ${((cached[cached.length-1] && new Date(cached[cached.length-1].timestamp)) || '').toISOString().split('T')[0]})`;
    ...
    fs.writeFileSync(`v4plus_15m_backtest_report_$${START_EQ}.json`, JSON.stringify(report, null, 2));
    console.log(`\n✓ Report saved to v4plus_15m_backtest_report_$${START_EQ}.json\n`);
...
runBacktest15m().catch(console.error);
